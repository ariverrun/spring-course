...
function BookView() {
  ...
  useEffect(() => {
    apiFetch(`/api/v1/book/${id}`)
      .then(response => response.json())
      .then(data => {
        setBook(data);
        return apiFetch(`/api/v1/comment?bookId=${id}`);
      })
      .then(response => response.json())
      .then(data => {
        setComments(data);
  ...
  const handleDeleteComment = (commentId) => {
    if (confirm('Удалить комментарий?')) {
      apiFetch(`/api/v1/comment/${commentId}`, { method: 'DELETE' })
        .then(() => apiFetch(`/api/v1/comment?bookId=${id}`))
        .then(response => response.json())
